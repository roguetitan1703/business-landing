  ...
  return (
    <Canvas>
      ...
      <PerspectiveCamera makeDefault position={[0, -4, 18]} fov={90} onUpdate={(self) => self.lookAt(0, 0, 0)}>
        ...
        <spotLight position={[5, -18, 0]} color="blue" angle={0.25} decay={0.75} distance={100} penumbra={-1} intensity={800} />
      </PerspectiveCamera>
      <context.Provider value={shake}>
        <CameraShake ref={shake} decay decayRate={0.95} maxYaw={0.02} maxPitch={0.005} yawFrequency={4} pitchFrequency={2} rollFrequency={2} intensity={0} />
        <Clouds limit={400} material={THREE.MeshLambertMaterial}>
          <Physics gravity={[0, 0, 0]}>
            <Pointer />
            <Puffycloud seed={10} position={[450, 0, 0]} />
            <Puffycloud seed={20} position={[400, 50, 0]} />
            <Puffycloud seed={30} position={[450, 0, 50]} />
            <Puffycloud seed={40} position={[400, 0, -50]} />
      ...
      <mesh scale={200}>
        <sphereGeometry />
        <meshStandardMaterial color="#111" roughness={0.9} side={THREE.BackSide} />
      </mesh>
...
function Puffycloud({ seed, vec = new THREE.Vector3(), ...props }) {
  ...
  useFrame((state, delta) => {
    ...
    if (impulse === 1) rig?.current?.setIntensity(0.5)
